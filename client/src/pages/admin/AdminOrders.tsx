...
export default function AdminOrders() {
    ...
    const fetchOrders = async () => { const d=await apiGet('/orders'); setOrders(d.orders||[]); setLoading(false); };

    const fetchPartners = async () => { const d=await apiGet('/delivery-partners'); setPartners((d.partners||[]).filter((p:any)=>p.isActive)); };
    ...
    const handleStatusChange = async (id: string, newStatus: string) => {
        try {
            const d = await apiPatch(`/orders/${id}/status`, { status: newStatus });
    ...
    const handleAssign = async () => {
        ...
        try {
            const d = await apiPatch(`/orders/${assignModal}/assign`, { deliveryPartnerId: selectedPartner });
            setOrders(prev => prev.map(o => o._id === assignModal ? d.order : o));
