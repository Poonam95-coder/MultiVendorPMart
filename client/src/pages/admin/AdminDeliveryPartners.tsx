...
export default function AdminDeliveryPartners() {
    ...
    const fetchPartners = async () => { try { const d=await apiGet('/delivery-partners'); setPartners(d.partners||[]); } finally { setLoading(false); } };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await apiPost('/delivery-partners', form);
            toast.success("Delivery partner created successfully!");
            setShowForm(false);
            setForm({ name: '', email: '', password: '', phone: '', vehicleType: 'bike' });
            await fetchPartners();
        } catch (e: any) {
            toast.error(e.message || "Failed to create delivery partner");
        } finally {
            setSaving(false);
        }
    };
    ...
    const toggleActive = async (id: string, isActive: boolean) => {
        try {
            await apiPatch(`/delivery-partners/${id}/toggle`, {});
