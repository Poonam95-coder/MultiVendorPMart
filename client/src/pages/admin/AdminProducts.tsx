...
import { PlusIcon, EditIcon, XIcon, Trash2Icon } from "lucide-react";
...
export default function AdminProducts() {
    ...
    const handleMarkOutOfStock = async (id: string, name: string) => {
        ...
        try {
            await apiPatch(`/products/${id}/stock`, { stock: 0 });
                                    ...
                                    <tr key={product._id} className="hover:bg-zinc-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <button onClick={() => handleMarkOutOfStock(product._id, product.name)} title="Mark Out of Stock" className="p-2 text-zinc-500 hover:text-amber-600 bg-zinc-100 hover:bg-amber-50 rounded-lg transition-colors">
                                                    <XIcon className="size-4" />
                                                </button>
                                                <button onClick={() => handleDelete(product._id, product.name)} title="Delete Product" className="p-2 text-zinc-500 hover:text-red-600 bg-zinc-100 hover:bg-red-50 rounded-lg transition-colors">
