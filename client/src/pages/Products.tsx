...
const Products = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (organic) qs.set("organic", organic);
      if (sort) qs.set("sort", sort);
      if (minprice) qs.set("minPrice", minprice);
      if (maxprice) qs.set("maxPrice", maxprice);
      qs.set("page", String(page));
      qs.set("limit", "12");
      const d = await apiGet(`/products?${qs}`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
           ...
           <nav className="flex items-center text-sm text-app-text-light mb-6">
            ...
            <span className="text-app-green font-medium">
              {activeCategory?activeCategory.name: "All Products"}</span>
            </nav>


            {/* Content for desktop screen for our applied filter */}
...
<div className="fixed bottom-0 left-0 right-0 bg-white
 z-50 rounded-t-2xl max-h-[80vh] overflow-y-auto 
 animate-slide-in-up">
