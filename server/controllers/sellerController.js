...
// Get seller profile and trust details
...
exports.getDashboard = async (req, res, next) => {
    try {
        ...
        // Get orders which contain products of this seller
        ...
        const completedOrders = allSellerOrders.filter(
        ...
        // Calculate revenue only from delivered seller items
        let revenue = 0;
        ...
        // Prepare recent orders for seller dashboard
...
exports.updateProduct = async (req, res, next) => {
    try {
        ...
        // Check that product belongs to this seller
        if (
            !product.seller ||
        ...
        if (isOrganic !== undefined) {
            product.isOrganic = Boolean(isOrganic);
        }
...
exports.deleteProduct = async (req, res, next) => {
    try {
        ...
        // Check product ownership before deleting
        if (
            !product.seller ||
            product.seller.toString() !==
            req.seller._id.toString()
...
exports.getOrders = async (req, res, next) => {
    try {
        ...
        const formattedOrders = orders
            .map((order) => {
                let sellerItems =
                    order.items.filter(matchSellerItem);
...
// Update seller status of an order item
    ...
    try {
        ...
        if (!allowedSellerStatuses.includes(status)) {
            return res.status(400).json({
                success: false,
                message:
                    `Invalid status. Must be one of: ${allowedSellerStatuses.join(", ")}`
            });
        }
        ...
        // Update matching items of this seller
        ...
        order.statusHistory.push({
            ...
            note:
                req.body.note ||
                `Seller updated item fulfillment status to ${status}`
        });


        // If every item is packed, update overall order status
        const allItemsPacked = order.items.every(
            (item) =>
                item.sellerStatus === "Packed" ||
                !item.seller
        );
        ...
        if (
            allItemsPacked &&
            ["Placed", "Confirmed", "Assigned"].includes(
                order.status
            )
        ) {
            order.status = "Packed";
