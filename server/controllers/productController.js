...
// Seller information used while populating products
...
// Add seller trust information to products
async function enrichProductsWithTrust(products) {
    ...
    // Get trust records of all sellers
    const trusts = await SellerTrust.find({
        seller: { $in: sellerIds }
    });
    ...
    return products.map((p) => {
        ...
        if (obj.seller && typeof obj.seller === "object") {
            ...
            // Use saved trust score or default score
            obj.seller.trustScore =
                t?.trustScore ?? (obj.seller.isVerified ? 85 : 50);
...
// Get only verified and active sellers
...
exports.getProducts = async (req, res, next) => {
    try {
        ...
        // Add trust score and trust summary
...
exports.getProduct = async (req, res, next) => {
    try {
        ...
        // Don't show products from inactive or unverified sellers
        if (p.seller && typeof p.seller === "object") {
            ...
            prodObj.seller.trustScore =
                trust?.trustScore ?? (p.seller.isVerified ? 85 : 50);
