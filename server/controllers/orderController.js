...
function addHistory(o, status, note = "") {
    o.status = status;
...
exports.place = async (req, res, next) => {
    try {
        ...
        for (const x of items) {
            ...
            // Only verified and active sellers can sell products
            if (p.seller && (!p.seller.isVerified || !p.seller.isActive)) {
                return res.status(400).json({
                    success: false,
                    message: `Product "${p.name}" is currently unavailable for purchase.`
                });
            }
        ...
        // Reduce product stock after order is created
        ...
        const order = await o.populate([
            "deliveryPartner",
            {
                path: "items.seller",
                select: "storeName rating isVerified"
            }
        ]);
...
exports.byId = async (req, res, next) => {
    try {
        ...
        // Check who is trying to access the order
        ...
        const isAssignedDelivery = Boolean(
            ...
            (
                o.deliveryPartner._id
                    ? o.deliveryPartner._id.toString()
                    : o.deliveryPartner.toString()
            ) === req.deliveryPartner._id.toString()
        );

        // Only related users can view the order
        if (!isAdmin && !isCustomer && !isSeller && !isAssignedDelivery) {
            return res.status(403).json({
                success: false,
                message: "Access denied: not authorized for this order"
            });
        }
...
exports.status = async (req, res, next) => {
    try {
        ...
        addHistory(
            o,
            s,
            req.body.note || `Status changed to ${s}`
        );

        // Generate OTP when order goes out for delivery
        if (s === "Out for Delivery" && !o.deliveryOtp) {
            o.deliveryOtp = generateOTP();
        }
...
exports.assign = async (req, res, next) => {
    try {
        const p = await DeliveryPartner.findById(
        ...
        if (!p || !p.isActive) {
            return res.status(400).json({
                success: false,
                message: "Active delivery partner required"
            });
        }
...
exports.cancel = async (req, res, next) => {
    try {
        ...
        // Return product stock after cancellation
...
exports.deliveryStatus = async (req, res, next) => {
    try {
        ...
        // Make sure this order belongs to the logged-in partner
        if (
            !o.deliveryPartner ||
            o.deliveryPartner.toString() !==
            req.deliveryPartner._id.toString()
            ...
            return res.status(403).json({
                success: false,
                message: "Order is not assigned to this delivery partner"
            });
        }
        ...
        addHistory(
            o,
            req.body.status,
            req.body.note || "Delivery status updated"
        );
...
exports.complete = async (req, res, next) => {
    try {
        ...
        if (otp !== String(o.deliveryOtp).trim()) {
            return res.status(400).json({
                success: false,
...
// Cancel an order from delivery partner side
exports.deliveryCancel = async (req, res, next) => {
    try {
        ...
        // Add cancelled items back to stock
        for (const i of o.items) {
            await Product.findByIdAndUpdate(
                i.product,
                {
        ...
        addHistory(
            o,
            "Cancelled",
            req.body.reason || "Delivery cancelled"
        );
