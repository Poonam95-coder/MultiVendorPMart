...
// Send only required user details in response
const safe = (u) => ({
    _id: u._id,
    name: u.name,
    email: u.email,
...
// Store login token in browser cookie
...
exports.register = async (req, res, next) => {
    try {
        ...
        // Normalize email before checking or saving it
...
exports.login = async (req, res, next) => {
    try {
        ...
        // Get user along with password for comparison
        ...
        // Decide role according to user account
...
exports.sellerRegister = async (req, res, next) => {
    try {
        ...
        // Create seller profile linked with the user
        const seller = await Seller.create({
            user: u._id,
            storeName: String(storeName).trim(),
            storeDescription: storeDescription
                ? String(storeDescription).trim()
                : "",
            email: normalizedEmail,
            phone: phone ? String(phone).trim() : "",
        ...
        // Create initial trust record for the seller
...
exports.sellerLogin = async (req, res, next) => {
    try {
        ...
        // Check whether the user has a seller profile
        const seller = await Seller.findOne({
            user: u._id
        });
...
exports.deliveryLogin = async (req, res, next) => {
    try {
        ...
        const p = await DeliveryPartner.findOne({
            email: normalizedEmail
        }).select("+password");
        ...
        // Inactive delivery partners cannot login
        if (!p.isActive) {
            return res.status(403).json({
                success: false,
                message: "Delivery account is currently inactive"
            });
        }
