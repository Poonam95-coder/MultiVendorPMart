...
/**
 * Rule-based NLP sentiment analyzer
 * Works even when AI analysis is not available
 */
function analyzeRuleBased(text, rating) {
    if (!text || typeof text !== "string") {
        const ratingScore = rating
            ? Math.min(100, Math.max(0, (rating / 5) * 100))
            : 50;
    ...
    // Convert review into simple words
    const tokens = text
        .toLowerCase()
        .replace(/[^a-z0-9'\s]/g, " ")
        .split(/\s+/)
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        // Check previous word for intensifier or negation
        if (i > 0) {
    ...
    // Convert text sentiment from -1..+1 to 0..100
    let textScore = 50;
    ...
    if (wordCount > 0) {
        ...
        textScore = 50 + (normalized * 50);
    }


    // Combine review text and star rating
...
/**
 * Optional Gemini AI sentiment analysis
 * Runs only when GEMINI_API_KEY is available
 */
...
Do not include code markdown or any other text.`;
        ...
        console.warn(
            "Gemini sentiment analysis failed, using robust fallback:",
            e.message
        );
    }
...
async function analyzeSentiment(text, rating) {
    try {
        // Try Gemini AI first
        const aiResult =
            await analyzeWithGemini(text, rating);

        if (aiResult) {
            return aiResult;
        }
    } catch (e) {
        // Use rule-based analysis if AI fails
    }
