...
// Weights used for Seller Trust Score calculation
const TRUST_WEIGHTS = {
    VERIFICATION: 0.20,
    RATING: 0.30,
...
// Generate trust summary based on seller metrics
...
// Recalculate and save trust score for a seller
async function recalculateSellerTrust(sellerId) {
    try {
        ...
        // 2. Get all reviews of this seller
        ...
        if (totalReviews > 0) {
            ratingScore = Math.min(
                100,
                Math.max(0, (avgRating / 5) * 100)
            );
        } else if (seller.rating > 0) {
            ratingScore = Math.min(
                100,
                Math.max(0, (seller.rating / 5) * 100)
            );
        }


        // 4. Calculate review sentiment score
        let reviewSentimentScore = 70;

        if (totalReviews > 0) {
            reviewSentimentScore = Math.min(
                100,
                Math.max(
                    0,
                    totalSentimentScoreSum / totalReviews
                )
            );
        }
        ...
        // 5. Complaint score
        // Complaint module is currently clean/unregistered
        const totalComplaints = 0;
        const resolvedComplaints = 0;
        const complaintScore = 100;


        // 6. Calculate final weighted trust score
        const rawScore =
        ...
        // 8. Create or update SellerTrust record
        const sellerTrust = await SellerTrust.findOneAndUpdate(
            ...
            },
            {
                ...
                reviewSentimentScore:
                    Math.round(reviewSentimentScore),
        ...
        // Update seller rating from latest reviews
        if (totalReviews > 0) {
        ...
        console.error(
            "Error recalculating seller trust score:",
            error
        );
...
// Get existing trust record or create one if missing
async function getSellerTrust(sellerId) {
    try {
        let trust = await SellerTrust.findOne({
            seller: sellerId
        });
        ...
        console.error(
            "Error fetching seller trust record:",
            error
        );
